...
func TestMapStringerCreation(t *testing.T) {
	m := NewStringer[KeyInfo, Animal]()
	if m.shards == nil {
		t.Error("map is null.")
	}
	elephant1 := Animal{"elephant"}
	elephantKey1 := KeyInfo{city: "c1", name: "elephant"}

	elephant2 := Animal{"elephant2"}
	elephantKey2 := KeyInfo{city: "c", name: "elephant2"}

	m.Set(elephantKey1, elephant1)
	m.Set(elephantKey2, elephant2)

	if m.Count() != 2 {
		t.Error("map should contain exactly two elements.")
	}
	e1, ok := m.Get(elephantKey1)
	if !ok {
