...
func (ms *ConcurrentMapShared[K, V]) GetItemsUnsafe() map[K]V {
	return ms.items
}
...
// GetShard returns shard under given key
func (m ConcurrentMap[K, V]) GetShards() []*ConcurrentMapShared[K, V] {
	return m.shards
}
...
func (m ConcurrentMap[K, V]) ItemsSimple() map[K]V {
	tmp := make(map[K]V)
	// Insert items to temporary map.
	for _, cm := range m.GetShards() {
		cm.RLock()
